...
import languages
from tools.helpers import limited_print


class Publication():
    def __init__(self, id, language=None, text_areas=None, genre=None, period=None, object_type=None,
                 translation_source=None, src_url=None):
        self.id = id
        self.text_areas: list = list() if text_areas is None else text_areas
...
def get_atf():
    ...
    print("Parsing atf")
    parsed_atf = parse_atf(atf)
    limited_print(10 ** 100, parsed_atf)
    return parsed_atf
...
def sanitize_object_type(x):
    return str(x).lower().replace(" (see object remarks)", "").replace(" (not impression)", "").replace(" & envelope",
                                                                                                        "").replace(
        " in envelope", "").replace("prismatic cylinder", "prism").strip()
...
genres = {
    ...
    'private-votive',
    'incantation-ritual',
    'priestly-letter', 'extispicy-query', 'legal-transaction', 'extispicy-report', 'administrative-letter', 'decree',
    'prophecy',
    'scholarly',
    'votive-donation',
    'eponym-chronicle', 'hemerological', 'royal-inscription', 'lexical-mathematical', 'appointment',
    'astronomical-diary',
...
def get_genre(x):
    if x is None:
        return "other-genre"
    g = str(x).lower().replace("?", "").replace("(see subgenre)", "").replace("(modern)", "").replace("(seal)",
                                                                                                      "").replace("/",
                                                                                                                  "-").replace(
    ...
    match g:
        case 'literary-work':
            return 'literary'
        case 'scholarly-letter':
            return 'scholarly'
        case 'lexical-mathematical':
            return 'mathematical'
        case "other" | "nan":
            return "other-genre"
        case "gift" | "historical":  # 1 offs
            return "other-genre"
        case 'lexical-text':
            return 'lexical'
        case 'administrative-record' | 'administrative-letter':
            return 'administrative'
        case 'astrological-report':
            return 'astrological'
        case 'royal-metal-inscription' | 'royal-stone-inscription' | 'inscription-on-a-perle':
            return 'royal-inscription'
        case g if g in genres:
            return g
...
def wrap_paragraph(paragraph, lines, src_lang, wmax_num_tokens, tgt_lang, avg_src_chars_per_token=1.8713256996006793,
